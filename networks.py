...
def define_G(input_nc, output_nc, ngf, norm='batch', use_dropout=False, init_type='normal', init_gain=0.02,
  ...
  net = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=15)
...
class ResnetGenerator(nn.Module):
  def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=15,
    ...
    for i in range(n_blocks):
      model += [
        ResBlock(ngf * 4, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias)]
...
class GANLoss(nn.Module):
  ...
  def __call__(self, input, target_is_real):
    ...
    if config.x:
      if target_tensor[0,0,0,0] == 0:
        target_tensor += torch.cuda.FloatTensor(12, 1, 30, 30).uniform_(0, 0.01)
      else:
        target_tensor -= torch.cuda.FloatTensor(12, 1, 30, 30).uniform_(0, 0.01)
